...
use libp2p::{swarm::Swarm, PeerId, gossipsub::IdentTopic as Topic};
use std::str::FromStr;
use tokio::sync::mpsc;
use uuid::Uuid;
...
use crate::federation::send_federated_message;
use crate::psi;
...
pub struct AppState {
    ...
    pub groups: Arc<tokio::sync::Mutex<Vec<Group>>>,
}
...
pub async fn post_message(
    ...
    if let Ok(group_id) = Uuid::from_str(&recipient_user_id) {
        ...
        if let Some(group) = groups.iter().find(|g| g.id == group_id) {
            ...
            state.swarm.lock().await.behaviour_mut().publish_message(&topic, payload.plaintext)?;
            return Ok(());
        }
    }

    if let Ok(peer_id) = PeerId::from_str(&recipient_user_id) {
        if state.swarm.lock().await.is_connected(&peer_id) {
            let topic = Topic::new("connexa-messages");
            state.swarm.lock().await.behaviour_mut().publish_message(&topic, payload.plaintext)?;
        } else {
            // Not connected to peer, fallback to store and forward
            let message = EncryptedMessage {
                ciphertext: payload.plaintext.clone(),
                sender_blind: user_id.to_string().into_bytes(),
                dh_ratchet_pub: vec![],
                message_number: 0,
        ...
        let servers_guard = state.federation_state.servers.read().await;
        let server = servers_guard.iter().find(|s| s.name == server_name);
...
pub fn app(store: Arc<MessageStore>, federation_state: Arc<FederationState>, swarm: Arc<tokio::sync::Mutex<Swarm<MyBehaviour>>>, storage: Arc<Storage>, oprf_key: Arc<EphemeralSecret>, message_queue_sender: mpsc::Sender<EncryptedMessage>, groups: Arc<tokio::sync::Mutex<Vec<Group>>>) -> Router {
    Router::new()
